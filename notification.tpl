		...
		<div class="notif-content">
			<div class="first-line">
				<div class="row">
					{{#show_author}}
						<div class="content-img item author" onclick="closeNotif()">	
							<a class="prevent-link not-prevented" onclick="ajaxQuery( {{#author_type}}'channel'{{/author_type}}{{^author_type}}'user'{{/author_type}} , 'wall' , {{author.id}} , event )" href="../{{#author_type}}channel{{/author_type}}{{^author_type}}id{{/author_type}}{{author.id}}/wall">
								<div class="img" style="background:url('{{main_set.root}}/{{author.img}}');background-repeat:no-repeat;background-position: 50% 50%;background-size:cover;"></div>
							</a>
						</div>
					{{/show_author}}
			...
			{{#form}}
				...
				{{#type_10}}
					<div class="content-form" onclick="closeNotif()">
						<a class="prevent-link not-prevented" onclick="ajaxQuery( 'channel' , 'wall' , {{channel.id}} , event )" href="../channel{{author.id}}/wall">
							<div class="form-descr channel">
								<div class="descr-item">
									<span class="icon"></span>
						...
						<input type="button" class="input-btn-object" value="{{langs.accept}}" onclick="request( 32 , {{channel.id}} , event )"
						><input type="button" class="simple-btn-object" value="{{langs.remove}}" onclick="request( 33 , {{channel.id}} , event )">
					</div>
				{{/type_10}}
				</div>
			{{/form}}
