...
{{#channel_set}}
	...
	<form method="post" action="../php_requests/settings.php" id="channel-set-form">
		<input type="hidden" name="query" value="3">
		<input type="hidden" name="channel" value="{{channel.id}}">
		<input class="input name" name="name" placeholder="{{langs.name}}" value="{{channel.name}}">
		<textarea rows="2" name="description" class="input descr" placeholder="{{langs.descr}}">{{channel.description}}</textarea>
		<label class="check" ><input class="checkbox" type="checkbox" {{#channel.secret}}checked{{/channel.secret}} name="secret">{{langs.private_channel}}</label>
		<div class="input-btn-object button" onclick="channelUpdate()">{{langs.save_ch}}</div>
	</form>
...
{{#channel_admin}}
	...
	<div class="block">
		{{#admin}}
			<div class="user" data-id="{{id}}">
				<div class="request" onclick="request( 26 , {{channel.id}} , event )"></div>
	...
	<input placeholder="{{langs.search_for_admin}}" onkeyup="adminSearchSet({{channel.id}})" class="input">
...
{{#channel_req}}
	...
	{{#req_user}}
		<div class="user" data-id="{{id}}">
			<div class="request" onclick="request( 27 , {{channel.id}} , event )"></div>
			<div class="request" onclick="request( 28 , {{channel.id}} , event )"></div>
			<a class="prevent-link not-prevented" onclick="ajaxQuery( 'user' , 'wall' , {{id}} , event )" href="../id{{id}}/wall">
				<div class="img" style="background:url('../{{img}}');background-repeat:no-repeat;background-position: 50% 50%;background-size:cover;"></div>
			</a>
			<a class="prevent-link not-prevented" onclick="ajaxQuery( 'user' , 'wall' , {{id}} , event )" href="../id{{id}}/wall">
...
{{#channel_banned}}
	...
	{{#ban}}
		<div class="user" data-id={{id}}>
			<div class="request" onclick="request( 29 , {{channel.id}} , event )"></div>
			<a class="prevent-link not-prevented" onclick="ajaxQuery( 'user' , 'wall' , {{id}} , event )" href="../id{{id}}/wall">
				<div class="img" style="background:url('../{{img}}');background-repeat:no-repeat;background-position: 50% 50%;background-size:cover;"></div>
			</a>
			<a class="prevent-link not-prevented" onclick="ajaxQuery( 'user' , 'wall' , {{id}} , event )" href="../id{{id}}/wall">
	...
	<input placeholder="{{langs.search_by_id}}" onkeyup="banSearchSet({{channel.id}})" class="input">
	<div class="block search"></div>
{{/channel_banned}}
...
{{#channel_del}}
	<div class="del-form">
		...
		<div class="input-btn-object button" onclick="deleteGroup({{channel.id}})">{{langs.delete}}</div>
	</div>
{{/channel_del}}
